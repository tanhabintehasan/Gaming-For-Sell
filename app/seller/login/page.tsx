...
export default function SellerLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        if (data.data.level !== 'SELLER' && data.data.level !== 'ADMIN') {
          toast.error('该账号没有打手权限，请使用用户端登录')
          setLoading(false)
          return
        }
        toast.success('登录成功')
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative">
      ...
      <Card className="w-full max-w-md glass-card border-[rgba(0,245,255,0.15)] relative overflow-hidden">
        ...
        <CardHeader className="text-center relative z-10 pt-10">
          ...
          <CardDescription className="text-[rgba(180,200,255,0.6)]">打手专用工作台入口</CardDescription>
        </CardHeader>
        <CardContent className="relative z-10">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <Button
              type="submit"
              className="w-full h-12 rounded-xl font-bold text-[#050810] bg-gradient-to-r from-[#00f5ff] to-[#00c2cc] hover:brightness-110 border-0 shadow-[0_12px_28px_rgba(0,245,255,0.25)] hover:shadow-[0_16px_36px_rgba(0,245,255,0.35)] transition-all"
              disabled={loading}
            >
              {loading ? '登录中...' : '登录'}
            </Button>
          ...
          <div className="mt-6 flex items-center justify-between text-sm">
            <Link href="/login" className="text-[#00f5ff] hover:text-[#7df9ff] transition-colors flex items-center gap-1">
